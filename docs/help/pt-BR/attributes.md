...
Os botões aqui fazem as duas de uma vez. Para Word, Excel, JSON e `.strings`, o Gitcito
...
do app — então esses quatro funcionam sem instalar nada. O resto ainda precisa de
...
| `strings` | nada — vem com o Gitcito | Diffs por linha de um `.strings` UTF-16, que o git chama de binário |
| `pdf` | `pdftotext` (poppler) | Diffs de texto em `.pdf` |
| `exif` | `exiftool` | O que mudou numa imagem, quando os pixels são opacos |

### O que morde projetos iOS

`Localizable.strings` é UTF-16 durante quase toda a história do Xcode, e UTF-16
é cheio de bytes NUL, então o git o chama de binário e não mostra **nada**:
...
E é justamente o arquivo em que ver qual string alguém mexeu importa mais. O
driver `strings` o decodifica só para o diff — lendo a marca de ordem de bytes
em vez de supô-la, para que um `.strings` moderno em UTF-8 passe intacto em vez
de virar caracteres embaralhados.

String Catalogs (`.xcstrings`, Xcode 15 em diante) são JSON, e o driver `json`
cobre eles: ordena as chaves, então uma tradução adicionada no topo deixa de
reescrever o arquivo inteiro no diff.
