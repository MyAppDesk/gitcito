...
- **Timelapse** — watch the whole repository grow. Every file is a dot placed by its top-level folder, born when it is added, pulsing when a commit touches it, swelling as it gets edited again and again, fading out when deleted — with the date, author, subject and running commit/file/author counts on screen. Play/pause, 4×–32× speed and a slider that seeks by replaying (so scrubbing back is exact). **Export video** records the canvas end to end and writes a `.webm` wherever you choose — recorded in-page with `MediaRecorder`, so there is no encoder to install, no ffmpeg and nothing sent anywhere. From the tools menu or `⌘K`.
...
- **Mission control** — every repository of the workspace on one screen, ordered by what actually needs you: a repo stuck mid-rebase or with conflicts first, then work to pull or push, then dirty trees, then the quiet ones. Each row carries the branch and its upstream, ↑ahead / ↓behind, uncommitted / untracked counts, stashes, open PRs (when already loaded), a **14-day commit sparkline** and how long since the last commit — expand it to see exactly which commits are waiting to be pushed and which files are dirty. The status pills double as **filters**, there are **urgency / name / activity** orderings, a workspace-wide tally (to push, to pull, dirty, commits this fortnight), **tick several repos** to fetch them or pull just the ones that are behind, and it **refreshes itself** while it is open. Keyboard throughout: `↑`/`↓` (or `j`/`k`) walk the list, `Enter` opens, `f` fetches, `p` pulls, `space` expands, `/` jumps to the filter. Reading it is purely local — one `git status` per repo, no network, no tokens — so opening the dashboard never authenticates anywhere; fetching is always an explicit act. It is a **view, not a tab**: the gauge next to the workspace name toggles it (also in the workspace menu and `⌘K`), and clicking any tab returns you to your work.
