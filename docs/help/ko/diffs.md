...
| **연동** (분할 보기, 줄 바꿈 끔) | 양쪽을 함께 가로로 스크롤합니다 — 끄면 각 열이 따로 움직입니다 |
...
줄 바꿈이 꺼져 있으면 양쪽은 기본적으로 **연동**되어 가로로 스크롤되므로 왼쪽 90번째
칸이 오른쪽 90번째 칸 위에 놓입니다. 들여쓴 블록과 들여쓰지 않은 블록, 모든 줄을 밀어낸
이름 변경처럼 양쪽이 어긋났을 때는 연동을 풀고 각 절반을 자기 내용이 있는 곳에 두세요.
세로 스크롤은 두 경우 모두 공유되며, 행이 서로 마주 보게 하는 것이 바로 그것입니다.
